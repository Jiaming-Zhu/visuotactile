...
def compute_losses(
    ...
    if reg_type == "polarization":
        ...
        reg_loss = torch.tensor(0.0, device=g.device)
...
def build_model(cfg: Dict, args: argparse.Namespace, mass_classes: int, stiffness_classes: int, material_classes: int):
    return FusionModel(
...
def train(args: argparse.Namespace) -> None:
    ...
    model = build_model(vars(args), args, mass_classes, stiffness_classes, material_classes).to(device)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Online-prefix training script based on the gating fusion model")
    ...
    parser.add_argument("--save_dir", type=str, default="outputs/fusion_gating_online")
