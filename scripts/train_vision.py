...
class VisionOnlyModel(nn.Module):
    def __init__(
        ...
        with torch.no_grad():
            ...
            feature_map = self.vis_backbone(dummy)
            num_vis_tokens = feature_map.shape[2] * feature_map.shape[3]

        self.cls_token = nn.Parameter(torch.randn(1, 1, fusion_dim))
        self.pos_emb = nn.Parameter(torch.randn(1, 1 + num_vis_tokens, fusion_dim))
    ...
    def forward(self, img: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        x = torch.cat([cls_token, v_tokens], dim=1)
...
class RoboticGraspDataset(Dataset):
    def __init__(self, split_dir: Path, image_size: int = 224) -> None:
    ...
    def _collect_samples(self) -> List[Sample]:
        ...
        for obj_class_dir in sorted(self.split_dir.iterdir()):
            ...
            for episode_dir in sorted(obj_class_dir.iterdir()):
                ...
                if img_path.exists():
                    samples.append(Sample(img_path=img_path, labels=labels))
...
def apply_visual_dropout(images: torch.Tensor, visual_drop_prob: float = 0.0) -> torch.Tensor:
    if visual_drop_prob <= 0:
        return images
    bsz = images.size(0)
    device = images.device
    vis_mask = (torch.rand(bsz, device=device) < visual_drop_prob).view(-1, 1, 1, 1).float()
    return images * (1 - vis_mask)
...
def compute_metrics(
    ...
    for batch_idx, batch in enumerate(iterator, start=1):
        ...
        if train_mode:
            images = apply_visual_dropout(images, visual_drop_prob)
            optimizer.zero_grad()
...
def build_loader(split_dir: Path, batch_size: int, num_workers: int, shuffle: bool, image_size: int) -> DataLoader:
    dataset = RoboticGraspDataset(split_dir=split_dir, image_size=image_size)
    return DataLoader(
        dataset,
...
def _plot_summary(results: Dict[str, Dict], output_dir: Path) -> None:
    ...
    fig.suptitle("Vision-Only Model Evaluation Summary", fontsize=14, fontweight="bold")
...
class LiveTrainingPlotter:
    ...
    def __init__(self, output_dir: Path):
        ...
        self.fig.suptitle("Vision-Only Training (Live)", fontsize=14, fontweight="bold")
...
def train(args: argparse.Namespace) -> None:
    ...
    train_loader = build_loader(train_dir, args.batch_size, args.num_workers, shuffle=True, image_size=args.image_size)
    val_loader = build_loader(val_dir, args.batch_size, args.num_workers, shuffle=False, image_size=args.image_size)
...
def eval_split(args: argparse.Namespace, split_name: str, checkpoint_path: Optional[Path] = None) -> Dict:
    ...
    device = resolve_device(args.device)
    loader = build_loader(split_dir, args.batch_size, args.num_workers, shuffle=False, image_size=args.image_size)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Training script for vision-only baseline")
    parser.add_argument("--mode", choices=["train", "eval"], default="train")
    parser.add_argument("--data_root", type=str, default="/home/martina/Y3_Project/Plaintextdataset")
    parser.add_argument("--save_dir", type=str, default="outputs/vision_model_clean")
